...
- yevea.com/cat es la tienda EN PRODUCCIÓN.
...
- productos.pieza_unica (checkbox en EditProducto, default true en alta nueva): qty=1 sin duplicar en carrito, 'Vendido' si stock≤0 (pero sigue publicado), dims en producto, stock 1 al crear. Desmarcarlo permite stock>1 (comportamiento estándar). Antes vivía en familias.tipofamilia (selector ya eliminado de EditFamilia); esa columna se conserva SOLO para el backfill de Init::migrateProductUniqueFlag (WHERE pieza_unica IS NULL, no pisa ediciones manuales posteriores — mismo patrón que backfillProductSlugs). Precio por medidas SOLO vía calc_config (sigue siendo de familia). Filtro de dimensiones del catálogo: visible si la lista tiene productos con dims.
